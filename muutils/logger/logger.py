...
from muutils.logger.headerfuncs import HeaderFunction, HEADER_FUNCTIONS
from muutils.logger.simplelogger import SimpleLogger, AnyIO, NullIO
...
class Logger(SimpleLogger):
	...
	def __init__(
		...
		if "error" not in self._streams:
			self._streams["error"] = LoggingStream("error", aliases = {"err", "except", "Exception", "exception", "exceptions", "errors"})
