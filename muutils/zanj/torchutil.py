...
class ConfiguredModel(
    ...
    def load(cls, obj: dict[str, Any], path: ObjectPath, zanj: ZANJ|None = None) -> "ConfiguredModel":
        ...
        # from muutils.zanj.loading import LOADER_MAP, get_item_loader
        # from pydbg import dbg
        # for k, v in tensored_state_dict.items():
        #     dbg('-----------------')
        #     dbg(v["__format__"])
        #     dbg({kk: type(vv) for kk, vv in v.items()})
        #     dbg(get_item_loader(v, path + ("state_dict", k), zanj))
        ...
        #     dbg(LOADER_MAP["torch.Tensor"].check(v, path + ("state_dict", k), zanj))
