...
Usage:
    python -m muutils.misc.typing_breakdown [OPTIONS]

Examples:
    python -m muutils.misc.typing_breakdown
    python -m muutils.misc.typing_breakdown --error-dir .meta/.type-errors
    python -m muutils.misc.typing_breakdown --top-n 15 --output .meta/typing-summary.txt
"""
...
class TypeCheckResult:
	...
	def filter_by(self, top_n: int | None) -> TypeCheckResult:
		"return a copy with errors sorted by count and filtered to top_n items (or all if None)"
...
def parse_mypy(content: str) -> TypeCheckResult:
	...
	pattern: re.Pattern[str] = re.compile(r"^(.+?):\d+: error: .+ \[(.+?)\]", re.MULTILINE)
	match: re.Match[str]
...
def parse_basedpyright(content: str) -> TypeCheckResult:
	...
	for line in content.splitlines():
		...
		# Check if this is an error/warning line
			...
			match: re.Match[str] | None = re.search(r"\s+.+:\d+:\d+ - (error|warning): .+ \((\w+)\)", line)
			if match:
...
def parse_ty(content: str) -> TypeCheckResult:
	...
	error_pattern: re.Pattern[str] = re.compile(r"^(error|warning)\[(.+?)\]:", re.MULTILINE)
	# Pattern for location: --> file:line:col
	location_pattern: re.Pattern[str] = re.compile(r"^\s+-->\s+(.+?):\d+:\d+", re.MULTILINE)
...
def main(error_dir: str, output_file: str, top_n: int | None = 10) -> None:
	...
	# Add header comment with top_n info
	if top_n is None:
		output_lines.append("# Showing all errors")
	else:
		output_lines.append(f"# Showing top {top_n} errors per category")
	output_lines.append("")

	# First, extract summary lines from each type checker
	checkers_files: List[Tuple[str, str]] = [
		("mypy", "mypy.txt"),
		("basedpyright", "basedpyright.txt"),
		("ty", "ty.txt"),
	]

	name: str
	filename: str
	for name, filename in checkers_files:
	...
	checkers: List[Tuple[str, str, Callable[[str], TypeCheckResult]]] = [
	...
	for name, filename, parser_fn in checkers:
		...
		# Convert to TOML
		breakdown: str = filtered_result.to_toml()
...
if __name__ == "__main__":
	...
	parser.add_argument(
		...
		type=str,
		default="10",
		help='Number of top items to show in each category (default: 10). Use "all" or negative number for all items.',
	)
