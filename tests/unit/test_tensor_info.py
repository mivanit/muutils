...
from muutils.tensor_info import array_summary, array_info, generate_sparkline
...
@pytest.mark.parametrize("options", option_dicts)#, ids=lambda opt: f'opts_{opt["fmt"]}_spark{opt["sparkline"]}_col{opt["colored"]}')
@pytest.mark.parametrize("input_type,tensor_requires_grad", input_params)#, ids=lambda p: f'{p[0]}_grad{p[1]}')
def test_array_summary_comprehensive(input_type: str, tensor_requires_grad: bool, options: dict) -> None:
	"""
	...
	if (hasattr(arr, "size") and arr.size == 0) or (torch is not None and isinstance(arr, torch.Tensor) and arr.numel() == 0):
		assert "empty" in summary_str.lower(), f"Expected 'empty' in summary for {input_type}"
	else:
		# For non-empty arrays, if the options ask for dtype and shape info, they should appear.
		if options["dtype"]:
			assert "dtype" in summary_str.lower(), f"Expected 'dtype' info in summary for {input_type}"
		if options["shape"]:
			assert "shape" in summary_str.lower(), f"Expected 'shape' info in summary for {input_type}"
		# For torch inputs with device info requested.
		if torch is not None and isinstance(arr, torch.Tensor) and options["device"]:
			assert "device" in summary_str.lower(), f"Expected 'device' info in summary for {input_type}"
		# For arrays with NaNs, if not empty, a warning should be present.
		if input_type.endswith("with_nan"):
			assert ("nan" in summary_str.lower() or "nAN" in summary_str), f"Expected NaN warning in summary for {input_type}"
		# Check that the equality character appears in the summary (or its an error)
		assert options["eq_char"] in summary_str or tensor_info.SYMBOLS[options["fmt"]]["warning"] in summary_str, f"Expected eq_char '{options['eq_char']}' in summary for {input_type}"
		# If stats are enabled, at least one statistic (e.g. mean) should appear.
		if options["stats"] and not (("empty" in summary_str.lower()) or ("all nan" in summary_str.lower())):
			# The symbol for mean depends on format.
			if options["fmt"] == "unicode":
				assert "μ" in summary_str, f"Expected unicode mean symbol in summary for {input_type}"
			elif options["fmt"] == "latex":
				assert r"\mu" in summary_str, f"Expected latex mean symbol in summary for {input_type}"
			elif options["fmt"] == "ascii":
				assert "mean" in summary_str, f"Expected ascii 'mean' in summary for {input_type}"
		# If sparkline is enabled and the input is non-empty (and not all NaN) then a sparkline should appear.
		if options["sparkline"] and summary_str and not (("empty" in summary_str.lower()) or ("all nan" in summary_str.lower())):
			# We expect at least one vertical bar or sparkline characters.
			assert "|" in summary_str or any(c in summary_str for c in "▁▂▃▄▅▆▇█_-~=#"), f"Expected sparkline in summary for {input_type}"
