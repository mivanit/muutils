...
@dataset_decorator(["small"])
def test_progress_bar_options(input_range, expected):
    for pbar_value in [None, "none", "tqdm", "spinner"]:
        result = run_maybe_parallel(square, input_range, True, pbar_kwargs={}, pbar=pbar_value)
        assert result == expected

@dataset_decorator(["small"])
@pytest.mark.parametrize("pbar_value", ["invalid_string", 123, []])
def test_invalid_pbar_value(input_range, expected, pbar_value):
    with pytest.raises((ValueError, TypeError)):
        run_maybe_parallel(square, input_range, False, pbar=pbar_value)
