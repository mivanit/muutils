...
class BenchmarkRunner:
    ...
    def benchmark_method(self, method_name: str, method_func: Callable, 
                        task_func: Callable, data: List[int], 
    ...
    def run_benchmark_suite(self, data_sizes: List[int], task_funcs: Dict[str, Callable],
                           runs_per_method: int = 3) -> pd.DataFrame:
        ...
        for data_size in data_sizes:
            ...
            for task_name, task_func in task_funcs.items():
                ...
                stats = self.benchmark_method(
                    "sequential", benchmark_sequential, task_func, test_data, runs_per_method
                )
                self._record_result("sequential", task_name, data_size, stats)
                
                # Pool.map
                stats = self.benchmark_method(
                    "pool_map", benchmark_pool_map, task_func, test_data, runs_per_method
                )
                ...
                imap_func = lambda f, d: benchmark_pool_imap(f, d, chunksize=chunksize)
                stats = self.benchmark_method(
                ...
                # Pool.imap_unordered
                imap_unord_func = lambda f, d: benchmark_pool_imap_unordered(f, d, chunksize=chunksize)
                stats = self.benchmark_method(
                    "pool_imap_unordered", imap_unord_func, task_func, test_data, runs_per_method
                )
                self._record_result("pool_imap_unordered", task_name, data_size, stats)
                
                # run_maybe_parallel (ordered)
                rmp_func = lambda f, d: benchmark_run_maybe_parallel(f, d, parallel=True)
                stats = self.benchmark_method(
                    "run_maybe_parallel", rmp_func, task_func, test_data, runs_per_method
                )
                self._record_result("run_maybe_parallel", task_name, data_size, stats)
                
                # run_maybe_parallel (unordered)
                rmp_unord_func = lambda f, d: benchmark_run_maybe_parallel(f, d, parallel=True, keep_ordered=False)
                stats = self.benchmark_method(
                    "run_maybe_parallel_unordered", rmp_unord_func, task_func, test_data, runs_per_method
                )
                self._record_result("run_maybe_parallel_unordered", task_name, data_size, stats)
        
        return self._create_dataframe()
    
    def _record_result(self, method: str, task: str, data_size: int, stats: Dict[str, float]):
    ...
    def _create_dataframe(self) -> pd.DataFrame:
        ...
        sequential_times = df[df['method'] == 'sequential'][['task', 'data_size', 'mean_time']]
        sequential_times = sequential_times.rename(columns={'mean_time': 'sequential_time'})
...
def plot_speedup_by_data_size(df: pd.DataFrame, task_type: str = None, save_path: str = None):
    ...
    for method in plot_df['method'].unique():
        ...
        ax.plot(method_df['data_size'], method_df['speedup'], 
...
def plot_timing_comparison(df: pd.DataFrame, data_size: int = None, save_path: str = None):
    ...
    pivot_df = plot_df.pivot_table(
        index='task', columns='method', values='mean_time'
    )
    
    ax = pivot_df.plot(kind='bar', figsize=(12, 6), rot=0)
    ax.set_ylabel('Time (seconds)')
    ax.set_title(f'Timing Comparison{f" (Data Size: {data_size})" if data_size else ""}')
...
def plot_efficiency_heatmap(df: pd.DataFrame, save_path: str = None):
    ...
    sns.heatmap(pivot_df, annot=True, fmt='.2f', cmap='YlOrRd', 
                vmin=0, center=1, cbar_kws={'label': 'Speedup'})
...
def print_summary_stats(df: pd.DataFrame):
    ...
    best_by_task = df[df['method'] != 'sequential'].groupby('task').apply(
    ...
    avg_speedup = df[df['method'] != 'sequential'].groupby('method')['speedup'].agg(['mean', 'std'])
...
def main():
    ...
    print(f"Saved timing comparison to timing_comparison.png")
